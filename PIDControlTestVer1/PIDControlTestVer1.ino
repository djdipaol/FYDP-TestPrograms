...
int Kp = 1; //These values can change based on testing
int Ki = 0;
int Kd = 0;
...
void loop() {
  ...
  //Prints for serial plotter
  Serial.println(currSpeed);
  Serial.print(" ");
  
  setPWM(currSpeed); //read the speed using the hall effect tachometer, CHECK THIS
  if(counter == 0)
  {
    counter = millis(); //read the current time
  }
...
double getSpeed()
{
  ...
  while(true){
    ...
    if(micros()-start>30000000.0){ //check if stopped, may need to reduce time
      break;
    }
  }
